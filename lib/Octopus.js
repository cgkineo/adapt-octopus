...
export default class Octopus {
	...
	static async runRecursive(opts) {
		Promise.all(['components', 'extensions', 'menu', 'theme'].map(async f => {
			...
			return Promise.all(contents.map(async c => {
				...
				const octopus = new Octopus({ cwd: pluginDir, inputId });
				await octopus.start();
			}));
		}));
	}
	...
	constructor({ inputPath = 'properties.schema', inputId, cwd }) {
	...
	async write() {
		...
		if(this.isCli) console.log(`Written to ${this.outputPath}`);
	}
